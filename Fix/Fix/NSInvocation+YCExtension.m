...
#import "NSInvocation+YCExtension.h"
#import <UIKit/UIKit.h>
#import <objc/runtime.h>

@implementation NSInvocation (YCExtension)

- (void)setDd_returnValue_obj:(id)dd_returnValue_obj
{
    objc_setAssociatedObject(self, @selector(dd_returnValue_obj), dd_returnValue_obj, OBJC_ASSOCIATION_RETAIN_NONATOMIC);
#define SET_RETURN_VALUE_OBJ_TYPE_METHOD(type, method) \
else if(strcmp(returnType, @encode(type)) == 0) \
{ \
type result = [dd_returnValue_obj method]; \
...
#define SET_RETURN_VALUE_OBJ_TYPE(type) SET_RETURN_VALUE_OBJ_TYPE_METHOD(type, type ## Value)
    ...
    if(strcmp(returnType, @encode(id)) == 0) {
        [self setReturnValue:&dd_returnValue_obj];
    } else if(strcmp(returnType, "?") == 0) {
        void * returnValuePointer = [dd_returnValue_obj pointerValue];
        [self setReturnValue:&returnValuePointer];
    } else if(strcmp(returnType, "*") == 0) {
        void * returnValuePointer = [dd_returnValue_obj pointerValue];
        [self setReturnValue:&returnValuePointer];
    } else if(strcmp(returnType, ":") == 0) {
        SEL returnValuePointer = [dd_returnValue_obj pointerValue];
        [self setReturnValue:&returnValuePointer];
    }
...
- (void)setDd_arguments:(NSArray *)dd_arguments
{
    for (int index = 0; index < dd_arguments.count; index++) {
        [self dd_setMyArgument:dd_arguments[index] atIndex:index];
    }
}

- (NSArray *)dd_arguments {
